			...
			if (node.tagName === 'INPUT' && node.type === 'file') {
				...
				if (node.hasAttribute(allowdirsAttr)) {
					...
					shadow.innerHTML = '<div style="border: 1px solid #999; padding: 3px; width: 235px; box-sizing: content-box; font-size: 14px; height: 21px;">'
						...
						+ '<a id="clear" title="Clear selection" style="text-decoration: none; float: right; margin: -3px -1px 0 0; padding: 3px; font-weight: bold; font-size: 16px; color:#999; box-sizing: content-box;">&times;</a>'
						+ '</div>'
						+ '</div>'
				...
				node.addEventListener('change', function () {
					...
					if (files.length > 0) {
						...
						// from file input drag and drop (webkitEntries)
						if (files[0] && files[0].isFile || files[0].isDirectory) {
							dir._items = files;
						} else {
